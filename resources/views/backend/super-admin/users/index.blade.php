...
@section('content')
    <div class="min-h-screen p-0 rounded-lg">
        ...
        <div class="bg-white rounded-xl shadow-md mb-8 border border-gray-200 overflow-hidden">
            {{-- Toggle Header --}}
            <button type="button" id="toggleCreateUserForm"
                class="w-full flex items-center justify-between px-6 py-5 bg-gray-50 hover:bg-gray-100 transition text-left">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full border border-blue-500 flex items-center justify-center text-blue-600">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                    </div>
                    <h3 class="text-2xl font-semibold text-gray-900">Create New Account</h3>
                </div>

                <svg id="toggleCreateUserIcon" class="w-5 h-5 text-gray-500 transition-transform duration-300" fill="none"
            ...
            {{-- Collapsible Body --}}
            <div id="createUserFormWrapper" class="hidden p-6 border-t border-gray-200">
                <form method="POST" action="{{ route('backend.super-admin.users.store') }}">
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" id="cancelCreateUserForm"
                            class="bg-gray-100 text-gray-700 px-6 py-2 rounded shadow hover:bg-gray-200 transition duration-150 ease-in-out">
                            Cancel
                        </button>
                        ...
                        @forelse($users as $user)
                            <tr class="hover:bg-gray-50 border-t">
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    ...
                                    {{-- Delete --}}
                                    <form action="{{ route('backend.super-admin.users.destroy', $user->id) }}" method="POST"
                                        class="inline-block"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const toggleBtn = document.getElementById('toggleCreateUserForm');
            const formWrapper = document.getElementById('createUserFormWrapper');
            const toggleIcon = document.getElementById('toggleCreateUserIcon');
            const cancelBtn = document.getElementById('cancelCreateUserForm');
