...
@section('title','Add Driving Licence')

@section('content')
<div class="flex items-center justify-between mb-6">
  <h2 class="text-3xl font-bold text-black">Add Driving Licence — {{ $staffProfile->user->full_name ?? '—' }}</h2>
...
<form method="POST" action="{{ route('backend.admin.staff-profiles.driving-licences.store', $staffProfile) }}"
      class="bg-white p-6 rounded-lg shadow border border-gray-200">
  @csrf
  @include('backend.admin.staff-driving-licences._form')
</form>
@endsection
