        ...
        <tbody>
          @forelse($docs as $d)
            <tr class="hover:bg-gray-50 border-t">
              ...
              <td class="px-4 py-2">
                <a href="{{ $d->url }}" target="_blank" class="text-blue-600 hover:underline">{{ $d->filename }}</a>
              </td>
              <td class="px-4 py-2">{{ $d->mime }}</td>
              <td class="px-4 py-2">{{ optional($d->created_at)->format('d M Y H:i') }}</td>
              <td class="px-4 py-2">{{ $d->uploaded_by ?? '—' }}</td>
              <td class="px-4 py-2 text-right space-x-2">
                <a href="{{ route('backend.admin.staff-profiles.documents.edit', [$staffProfile, $d]) }}"
                   class="text-blue-600 hover:underline">Edit</a>
                <form action="{{ route('backend.admin.staff-profiles.documents.destroy', [$staffProfile, $d]) }}"
                      method="POST" class="inline-block" onsubmit="return confirm('Delete this document? The file will be removed.')">
                  @csrf @method('DELETE')
          ...
          @empty
            <tr><td colspan="6" class="px-4 py-6 text-center text-gray-500">No documents uploaded.</td></tr>
          @endforelse
        </tbody>
      </table>
