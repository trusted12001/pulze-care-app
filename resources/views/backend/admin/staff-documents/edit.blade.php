...
<div class="flex items-center justify-between mb-6">
  <h2 class="text-3xl font-bold text-black">Edit Document — {{ $staffProfile->user->full_name ?? '—' }}</h2>
...
<form method="POST" enctype="multipart/form-data"
      action="{{ route('backend.admin.staff-profiles.documents.update', [$staffProfile, $document]) }}"
      class="bg-white p-6 rounded-lg shadow border border-gray-200">
  @csrf @method('PUT')
  @include('backend.admin.staff-documents._form', ['document' => $document, 'categories' => $categories])
</form>
@endsection
