                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            {{-- NEW: Rota Table column --}}
                            <th
                                class="px-4 sm:px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Rota Table
                            </th>
                            <th
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="text-sm sm:text-base font-medium text-gray-900">{{ $p->location->name }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-gray-500">{{ $p->start_date->diffInDays($p->end_date) + 1 }} days
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    @php
                                ...
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <a href="{{ route('backend.admin.rota-periods.table', $p) }}"
                                        class="inline-flex items-center justify-center gap-1 px-2.5 sm:px-3 py-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors duration-200"
                                        title="View Rota Table">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium {{ $badge }}">
                                        <span
                                            class="w-1.5 h-1.5 rounded-full {{ $p->status === 'published' ? 'bg-green-500' : ($p->status === 'locked' ? 'bg-gray-500' : 'bg-yellow-500') }}"></span>
                                        {{ ucfirst($p->status) }}
                                    </span>
                                ...
                                <div
                                    class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-2 border-t border-gray-100">
                                    <a href="{{ route('backend.admin.rota-periods.show', $p) }}"
                                    ...
                                    <a href="{{ route('backend.admin.rota-periods.table', $p) }}"
                                        class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 active:bg-emerald-200 transition-colors text-sm font-medium">
                                        <i class="ph ph-table"></i>
                                        Rota Table
                                    </a>
                                </div>
