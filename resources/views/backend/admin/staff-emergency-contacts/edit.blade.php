...
<div class="flex items-center justify-between mb-6">
  <h2 class="text-3xl font-bold text-black">Edit Emergency Contact — {{ $staffProfile->user->full_name ?? '—' }}</h2>
...
<form method="POST" action="{{ route('backend.admin.staff-profiles.emergency-contacts.update', [$staffProfile, $contact]) }}"
      class="bg-white p-6 rounded-lg shadow border border-gray-200">
  @csrf @method('PUT')
  @include('backend.admin.staff-emergency-contacts._form', ['contact' => $contact])
</form>
@endsection
