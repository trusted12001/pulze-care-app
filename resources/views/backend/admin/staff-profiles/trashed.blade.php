...
@section('content')
  ...
  <div class="min-h-screen p-3 sm:p-4 lg:p-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-2 flex items-center gap-2">
            ...
            <span>Trashed Staff Profiles</span>
          </h1>
          <p class="text-sm sm:text-base text-gray-600">View and manage deleted staff profiles</p>
        </div>
    ...
    @if($profiles->count())
      ...
      <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-4 sm:px-6 py-4 bg-gradient-to-r from-red-50 to-orange-50 border-b border-gray-200">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-900 flex items-center gap-2">
              <i class="ph ph-trash text-red-600"></i>
              <span>Deleted Staff Profiles</span>
            ...
            <div class="relative w-full sm:w-auto sm:min-w-[250px]">
              ...
              <input type="text" id="trashSearch" placeholder="Search trashed profiles..."
        ...
        <div class="hidden md:block overflow-x-auto">
          <table id="trashedProfilesTable" class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @foreach($profiles as $p)
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      <div
                        ...
                        {{ strtoupper(substr($p->user->first_name ?? 'U', 0, 1)) }}
                      </div>
                      <div>
                        <div class="text-sm sm:text-base font-medium text-gray-900">
                          {{ $p->user->full_name ?? '—' }}
                        </div>
                        <div class="text-xs text-gray-500">Deleted {{ $p->deleted_at->diffForHumans() }}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="px-4 sm:px-6 py-4">
                    <div class="text-sm text-gray-900">{{ $p->job_title ?? '—' }}</div>
                  </td>
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ $fmtDateTime($p->deleted_at) }}</div>
                    <div class="text-xs text-gray-500">{{ $p->deleted_at->diffForHumans() }}</div>
                  </td>
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-2">
                      <button type="button"
                        onclick="showRestoreModal({{ $p->id }}, '{{ addslashes($p->user->full_name ?? '—') }}', '{{ addslashes($p->user->email ?? '—') }}')"
                      ...
                      <button type="button"
                        onclick="showForceDeleteModal({{ $p->id }}, '{{ addslashes($p->user->full_name ?? '—') }}', '{{ addslashes($p->user->email ?? '—') }}')"
        ...
        <div class="md:hidden divide-y divide-gray-200">
          @foreach($profiles as $p)
            <div class="p-4 hover:bg-gray-50 transition-colors duration-150 trashed-profile-card">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between gap-2 mb-2">
                    <div class="min-w-0 flex-1">
                      ...
                      @if($p->job_title)
                        <p class="text-xs text-gray-600 mt-1">{{ $p->job_title }}</p>
                      @endif
                    </div>
                  </div>
                  <div class="mb-3">
                    <p class="text-xs text-gray-600">
                      ...
                      Deleted {{ $fmtDateTime($p->deleted_at) }}
                    </p>
                    <p class="text-xs text-gray-500 mt-1">{{ $p->deleted_at->diffForHumans() }}</p>
                  </div>
      ...
      <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-12 text-center">
          <div class="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-1">No Trashed Profiles</h3>
              <p class="text-sm text-gray-500">There are no deleted staff profiles to display.</p>
            </div>
            <a href="{{ route('backend.admin.staff-profiles.index') }}"
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Are you sure you want to restore this staff profile? The profile will be accessible again.
                  </p>
                  <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                    <p class="text-sm font-medium text-gray-900" id="restoreProfileName"></p>
                    <p class="text-xs text-gray-500 mt-1 break-all" id="restoreProfileEmail"></p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                <h3 class="text-lg leading-6 font-semibold text-gray-900" id="force-delete-modal-title">
                  Permanently Delete Staff Profile
                </h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    <strong class="text-red-600">Warning:</strong> This action cannot be undone. This will permanently
                    delete the staff profile and all associated data.
                  </p>
                  <div class="mt-4 p-3 bg-red-50 rounded-lg border border-red-200">
                    <p class="text-sm font-medium text-gray-900" id="forceDeleteProfileName"></p>
                    <p class="text-xs text-gray-500 mt-1 break-all" id="forceDeleteProfileEmail"></p>
                  </div>
                </div>
              </div>
            </div>
    ...
    {{-- Search Script --}}
    <script>
      function showRestoreModal(profileId, profileName, profileEmail) {
        const modal = document.getElementById('restoreModal');
        const form = document.getElementById('restoreForm');
        const nameElement = document.getElementById('restoreProfileName');
        const emailElement = document.getElementById('restoreProfileEmail');

        form.action = '{{ route("backend.admin.staff-profiles.restore", ":id") }}'.replace(':id', profileId);
        nameElement.textContent = profileName;
        emailElement.textContent = profileEmail;
      ...
      function showForceDeleteModal(profileId, profileName, profileEmail) {
        const modal = document.getElementById('forceDeleteModal');
        const form = document.getElementById('forceDeleteForm');
        const nameElement = document.getElementById('forceDeleteProfileName');
        const emailElement = document.getElementById('forceDeleteProfileEmail');

        form.action = '{{ route("backend.admin.staff-profiles.forceDelete", ":id") }}'.replace(':id', profileId);
        nameElement.textContent = profileName;
        emailElement.textContent = profileEmail;
        modal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
      ...
      document.getElementById('trashSearch').addEventListener('keyup', function () {
        ...
        const rows = document.querySelectorAll('#trashedProfilesTable tbody tr, .trashed-profile-card');
