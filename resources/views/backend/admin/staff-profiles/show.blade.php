...
@section('content')
    @php
        ...
        // Passport photo & initials for avatar
        $avatarUrl = $staffProfile->passport_photo_url ?? null;
        ...
        if ($user) {
            ...
            if ($nameString === '' && !empty($user->full_name)) {
                $nameString = $user->full_name;
            }

            $parts = preg_split('/\s+/', $nameString);
            $buf = '';
            foreach ($parts as $p) {
                if ($p === '')
                    continue;
                $buf .= mb_strtoupper(mb_substr($p, 0, 1));
                if (mb_strlen($buf) >= 2) {
                    break;
                }
            }
    ...
    <div class="max-w-6xl mx-auto py-4 sm:py-6 lg:py-8 px-3 sm:px-4 lg:px-6 xl:px-8">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex flex-col lg:flex-row lg:items-start gap-4 sm:gap-6 mb-4 sm:mb-6">
                ...
                <div class="flex items-start gap-4 flex-1">
                    ...
                    <div class="flex-shrink-0">
                        @if($avatarUrl)
                            <div class="relative">
                                <img src="{{ $avatarUrl }}" alt="Passport photo of {{ $user->full_name ?? 'staff member' }}"
                                    class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover border-2 border-blue-100 shadow-sm">
                            </div>
                        @else
                            <div
                                class="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-500 to-indigo-600 text-white text-xl sm:text-2xl font-semibold shadow-sm border border-white/60">
                                <span>{{ $initials }}</span>
                            </div>
                        @endif
                    </div>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
                    ...
                    {{-- Print --}}
                    <a href="{{ route('backend.admin.staff-profiles.print', $staffProfile->id) }}" target="_blank"
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-700 text-white rounded-lg hover:bg-gray-800 text-sm font-medium">
            ...
            @if (session('success'))
                <div
                    class="mb-4 sm:mb-6 p-3 sm:p-4 bg-green-50 border-l-4 border-green-500 text-green-800 rounded-lg shadow-sm">
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 sm:p-5 lg:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Job
                                        Title</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium break-words">
                                        {{ $val($staffProfile->job_title) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                    ...
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDate($staffProfile->date_of_birth) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Work
                                        Email</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium break-all">
                                        {{ $val($staffProfile->work_email) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 sm:p-5 lg:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Employment
                                        Status</label>
                                    <span
                                ...
                                <div>
                                    <label
                                        ...
                                        Type</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ ucfirst($val($staffProfile->employment_type)) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Engagement
                                        Basis</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ ucwords(str_replace('_', ' ', $val($staffProfile->engagement_basis))) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Hire
                                        Date</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDate($staffProfile->hire_date) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Start
                                        in
                                        Post</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDate($staffProfile->start_in_post) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">End
                                        in
                                        Post</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDate($staffProfile->end_in_post) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        ...
                                        Location</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $val(optional($staffProfile->workLocation)->name) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Line
                                        Manager</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $val(optional($staffProfile->lineManager)->full_name) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 sm:p-5 lg:p-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">DBS
                                        Number</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $val($staffProfile->dbs_number) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        ...
                                        At</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDate($staffProfile->dbs_issued_at) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Mandatory
                                        Training Completed</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDateTime($staffProfile->mandatory_training_completed_at) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">Right
                                        to
                                        Work Verified</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $fmtDateTime($staffProfile->right_to_work_verified_at) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">NMC
                                        PIN</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $val($staffProfile->nmc_pin) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5 sm:mb-2">GPhC
                                        PIN</label>
                                    <p class="text-sm sm:text-base text-gray-900 font-medium">
                                        {{ $val($staffProfile->gphc_pin) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-4">
                                <div class="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
                                    <p class="text-xl sm:text-2xl font-bold text-gray-900 break-words">
                                        {{ $fmtDate($staffProfile->created_at) }}
                                    </p>
                        ...
                        <div class="p-4 sm:p-5 lg:p-6">
                            <div class="space-y-2.5 sm:space-y-3 text-xs sm:text-sm">
                                ...
                                <div class="flex justify-between items-center gap-2">
                                    ...
                                    <span
                                        class="font-medium text-gray-900 break-words">{{ $fmtDate($staffProfile->created_at) }}</span>
                                </div>
                                <div class="flex justify-between items-center gap-2">
                                    <span class="text-gray-500">Updated:</span>
                                    <span
                                        class="font-medium text-gray-900 break-words">{{ $fmtDate($staffProfile->updated_at) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        @foreach($contracts as $c)
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-2">{{ ucfirst(str_replace('_', ' ', $c->contract_type ?? '—')) }}
                                                </td>
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                            ...
                            <div class="p-4">
                                ...
                                <p class="text-sm font-medium text-gray-900">
                                    {{ $payroll->ni_number ? '••••' . substr($payroll->ni_number, -4) : '—' }}
                                </p>
                            </div>
                        </div>
                    @endif
