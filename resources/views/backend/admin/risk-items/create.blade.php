  ...
  <div class="max-w-6xl mx-auto py-4 sm:py-6 lg:py-8 px-3 sm:px-4 lg:px-6">
    ...
    <div class="mb-6 sm:mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Add Risk Item</h1>
          <p class="text-sm sm:text-base text-gray-600">Create a new risk assessment item</p>
        </div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-4 sm:px-5 lg:px-6 py-3 sm:py-4 bg-gradient-to-r from-red-50 to-pink-50 border-b border-gray-200">
            <h2 class="text-base sm:text-lg font-semibold text-gray-900 flex items-center gap-2">
              ...
              <span>Risk Item Information</span>
            </h2>
          </div>
          ...
          <form method="POST"
            ...
            {{-- Profile Selection (if not nested) --}}
                  ...
                  @foreach($profiles as $p)
                    @php
                      ...
                      $suName = $su
                        ...
                        : null;
                    @endphp
                    <option value="{{ $p->id }}" @selected(old($profileFk, request('profile')) == $p->id)>
            ...
            @else
              ...
              <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div class="flex items-center gap-2 text-sm text-blue-800">
                  ...
                  <span><strong>Assessment:</strong> {{ $profile->title }}</span>
                </div>
              </div>
            @endunless
                ...
                <select name="risk_type_id" id="risk_type_id"
                  ...
                  @foreach($types as $t)
                    <option value="{{ $t->id }}" data-guidance="{{ e($t->default_guidance ?? '') }}"
                      data-matrix='@json($t->default_matrix ?? [])' @selected(old('risk_type_id', request('type')) == $t->id)>
                      {{ $t->name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div>
              <label class="block text-xs font-medium text-gray-700 uppercase tracking-wider mb-1.5 sm:mb-2">
                Context (where/when/trigger) <span class="text-red-500">*</span>
              </label>
            ...
            <div>
              ...
              <input type="text" name="hazard" value="{{ old('hazard') }}" placeholder="e.g., Slippery floor in bathroom"
                class="w-full bg-gray-50 border border-gray-300 rounded-lg px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors"
                required>
              @error('hazard')
                <p class="text-red-600 text-xs sm:text-sm mt-1">{{ $message }}</p>
            ...
            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5">
                ...
                <div>
                  ...
                  <input type="number" min="1" max="5" name="severity" id="severity" value="{{ old('severity', 3) }}"
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div class="p-4 sm:p-5 lg:p-6">
            <div id="matrix_box" class="text-sm text-gray-700">
              @if(isset($riskType) && $riskType->default_matrix)
                <div class="text-gray-500">Matrix will display here when a type is selected.</div>
              @else
