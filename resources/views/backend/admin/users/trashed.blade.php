...
@section('content')
    ...
    <div class="min-h-screen p-3 sm:p-4 lg:p-6">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-sm sm:text-base text-gray-600">View and manage deleted user accounts</p>
                </div>
        ...
        @if($users->count())
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="px-4 sm:px-6 py-4 bg-gradient-to-r from-red-50 to-orange-50 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="relative w-full sm:w-auto sm:min-w-[250px]">
                            ...
                            <input type="text" id="trashSearch" placeholder="Search trashed users..."
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table id="trashedUsersTable" class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($users as $user)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-500">Deleted {{ $user->deleted_at->diffForHumans() }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $fmtDateTime($user->deleted_at) }}</div>
                                        <div class="text-xs text-gray-500">{{ $user->deleted_at->diffForHumans() }}</div>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            <button type="button"
                                                onclick="showRestoreModal({{ $user->id }}, '{{ addslashes($user->full_name ?? $user->name) }}', '{{ addslashes($user->email) }}')"
                                            ...
                                            <button type="button"
                                                onclick="showForceDeleteModal({{ $user->id }}, '{{ addslashes($user->full_name ?? $user->name) }}', '{{ addslashes($user->email) }}')"
                ...
                <div class="md:hidden divide-y divide-gray-200">
                    @foreach($users as $user)
                        <div class="p-4 hover:bg-gray-50 transition-colors duration-150 trashed-user-card">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mb-3">
                                        <p class="text-xs text-gray-600">
                                            ...
                                            Deleted {{ $fmtDateTime($user->deleted_at) }}
                                        </p>
                                        <p class="text-xs text-gray-500 mt-1">{{ $user->deleted_at->diffForHumans() }}</p>
                                    </div>
            ...
            <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-12 text-center">
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 mb-1">No Trashed Users</h3>
                            <p class="text-sm text-gray-500">There are no deleted user accounts to display.</p>
                        </div>
                        <a href="{{ route('backend.admin.users.index') }}"
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Are you sure you want to restore this user account? The user will be able to access
                                        their account again.
                                    </p>
                                    <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                                        <p class="text-sm font-medium text-gray-900" id="restoreUserName"></p>
                                        <p class="text-xs text-gray-500 mt-1 break-all" id="restoreUserEmail"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div id="forceDeleteModal" class="hidden fixed inset-0 z-50 overflow-y-auto"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="hideForceDeleteModal()">
                </div>
                <div
                    class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        <strong class="text-red-600">Warning:</strong> This action cannot be undone. This
                                        will permanently delete the user account and all associated data.
                                    </p>
                                    <div class="mt-4 p-3 bg-red-50 rounded-lg border border-red-200">
                                        <p class="text-sm font-medium text-gray-900" id="forceDeleteUserName"></p>
                                        <p class="text-xs text-gray-500 mt-1 break-all" id="forceDeleteUserEmail"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        {{-- Search Script --}}
        <script>
            function showRestoreModal(userId, userName, userEmail) {
                const modal = document.getElementById('restoreModal');
                const form = document.getElementById('restoreForm');
                const nameElement = document.getElementById('restoreUserName');
                const emailElement = document.getElementById('restoreUserEmail');

                form.action = '{{ route("backend.admin.users.restore", ":id") }}'.replace(':id', userId);
            ...
            function showForceDeleteModal(userId, userName, userEmail) {
                const modal = document.getElementById('forceDeleteModal');
                const form = document.getElementById('forceDeleteForm');
                const nameElement = document.getElementById('forceDeleteUserName');
                const emailElement = document.getElementById('forceDeleteUserEmail');

                form.action = '{{ route("backend.admin.users.forceDelete", ":id") }}'.replace(':id', userId);
                nameElement.textContent = userName;
                emailElement.textContent = userEmail;
                modal.classList.remove('hidden');
                document.body.style.overflow = 'hidden';
            ...
            document.getElementById('trashSearch').addEventListener('keyup', function () {
                ...
                const rows = document.querySelectorAll('#trashedUsersTable tbody tr, .trashed-user-card');
