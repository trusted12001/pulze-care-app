...
@section('title','Add Disciplinary')

@section('content')
<div class="flex items-center justify-between mb-6">
  <h2 class="text-3xl font-bold text-black">Add Disciplinary — {{ $staffProfile->user->full_name ?? '—' }}</h2>
...
<form method="POST" action="{{ route('backend.admin.staff-profiles.disciplinary.store', $staffProfile) }}"
      class="bg-white p-6 rounded-lg shadow border border-gray-200">
  @csrf
  @include('backend.admin.staff-disciplinary-records._form')
</form>
@endsection
