...
<form action="{{ $route }}" method="POST" class="bg-white rounded-lg shadow p-4 space-y-4">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      ...
      <select name="service_user_id" class="border rounded w-full px-3 py-2" required>
        ...
        @foreach($serviceUsers as $su)
          <option value="{{ $su->id }}" @selected(old('service_user_id', $assessment->service_user_id ?? null)==$su->id)>
    ...
    <div>
      ...
      <input name="title"
             value="{{ old('title', $assessment->title ?? 'Comprehensive Risk Assessment') }}"
    ...
    <div>
      ...
      <input type="date"
             name="start_date"
             value="{{ old('start_date', optional($assessment?->start_date)->format('Y-m-d')) }}"
             class="border rounded w-full px-3 py-2">
    </div>
    ...
    <div>
      ...
      <input name="review_frequency"
             placeholder="e.g. 24 weeks"
             value="{{ old('review_frequency', $assessment->review_frequency ?? '') }}"
             class="border rounded w-full px-3 py-2">
    </div>

    {{-- Status on CREATE is controlled by the buttons (draft/publish). If you prefer a select, add it here. --}}
  </div>

  <div>
    <label class="block text-sm font-medium mb-1">Summary</label>
    <textarea name="summary" rows="3" class="border rounded w-full px-3 py-2">{{ old('summary', $assessment->summary ?? '') }}</textarea>
  </div>
  ...
  <div class="flex items-center gap-3">
    ...
    <button name="action" value="publish"
            ...
            onclick="return confirm('Publish and mark Active?')">
      Publish
    </button>
