  ...
  <div class="max-w-4xl mx-auto py-4 sm:py-6 lg:py-8 px-3 sm:px-4 lg:px-6">
    ...
    <div class="mb-6 sm:mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Edit Location</h1>
          <p class="text-sm sm:text-base text-gray-600">Update location information</p>
        </div>
      ...
      <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl p-4 sm:p-5 border border-blue-200">
        <div class="flex items-start gap-4">
          <div
            ...
            <i class="ph ph-buildings text-2xl sm:text-3xl"></i>
          </div>
          <div class="flex-1 min-w-0">
            <h2 class="text-lg sm:text-xl font-semibold text-gray-900 break-words mb-3">{{ $location->name }}</h2>
    ...
    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <form method="POST" action="{{ route('backend.admin.locations.update', $location) }}" class="p-4 sm:p-5 lg:p-6">
        @csrf
        @method('PUT')
