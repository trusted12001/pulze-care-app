...
@section('content')
  ...
  <div class="min-h-screen p-3 sm:p-4 lg:p-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div>
          <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-2 flex items-center gap-2">
            ...
            <span>Trashed Locations</span>
          </h1>
          <p class="text-sm sm:text-base text-gray-600">View and manage deleted locations</p>
        </div>
    ...
    @if($locations->count())
      ...
      <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-4 sm:px-6 py-4 bg-gradient-to-r from-red-50 to-orange-50 border-b border-gray-200">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="relative w-full sm:w-auto sm:min-w-[250px]">
              ...
              <input type="text" id="trashSearch" placeholder="Search trashed locations..."
        ...
        <div class="hidden md:block overflow-x-auto">
          <table id="trashedLocationsTable" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">City
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @foreach($locations as $loc)
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div class="text-xs text-gray-500">Deleted {{ $loc->deleted_at->diffForHumans() }}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="px-4 sm:px-6 py-4">
                    <div class="text-sm text-gray-900">{{ $loc->city ?? '—' }}</div>
                  </td>
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ $fmtDateTime($loc->deleted_at) }}</div>
                    <div class="text-xs text-gray-500">{{ $loc->deleted_at->diffForHumans() }}</div>
                  </td>
                  <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-2">
                      <button type="button" onclick="showRestoreModal({{ $loc->id }}, '{{ addslashes($loc->name) }}')"
                      ...
                      <button type="button" onclick="showForceDeleteModal({{ $loc->id }}, '{{ addslashes($loc->name) }}')"
        ...
        <div class="md:hidden divide-y divide-gray-200">
          @foreach($locations as $loc)
            <div class="p-4 hover:bg-gray-50 transition-colors duration-150 trashed-location-card">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between gap-2 mb-2">
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="text-xs text-gray-500 mt-1">
                        <span
                          class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-indigo-100 text-indigo-800 rounded text-xs">
                          {{ ucfirst(str_replace('_', ' ', $loc->type)) }}
                        </span>
                        @if($loc->city)
                          <span class="ml-2">{{ $loc->city }}</span>
                        @endif
                      </p>
                    </div>
                  </div>
                  <div class="mb-3">
                    <p class="text-xs text-gray-600">
                      ...
                      Deleted {{ $fmtDateTime($loc->deleted_at) }}
                    </p>
                    <p class="text-xs text-gray-500 mt-1">{{ $loc->deleted_at->diffForHumans() }}</p>
                  </div>
      ...
      <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-12 text-center">
          <div class="flex flex-col items-center gap-4">
            ...
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-1">No Trashed Locations</h3>
              <p class="text-sm text-gray-500">There are no deleted locations to display.</p>
            </div>
            <a href="{{ route('backend.admin.locations.index') }}"
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Are you sure you want to restore this location? The location will be accessible again.
                  </p>
                  <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                    <p class="text-sm font-medium text-gray-900" id="restoreLocationName"></p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    ...
                    delete the location and all associated data.
                  </p>
                  <div class="mt-4 p-3 bg-red-50 rounded-lg border border-red-200">
                    <p class="text-sm font-medium text-gray-900" id="forceDeleteLocationName"></p>
                  </div>
                </div>
              </div>
            </div>
    ...
    {{-- Search Script --}}
    <script>
      function showRestoreModal(locationId, locationName) {
        const modal = document.getElementById('restoreModal');
        const form = document.getElementById('restoreForm');
        const nameElement = document.getElementById('restoreLocationName');

        form.action = '{{ route("backend.admin.locations.restore", ":id") }}'.replace(':id', locationId);
      ...
      function showForceDeleteModal(locationId, locationName) {
        const modal = document.getElementById('forceDeleteModal');
        const form = document.getElementById('forceDeleteForm');
        const nameElement = document.getElementById('forceDeleteLocationName');

        form.action = '{{ route("backend.admin.locations.forceDelete", ":id") }}'.replace(':id', locationId);
        nameElement.textContent = locationName;
        modal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
      }
      ...
      document.getElementById('trashSearch').addEventListener('keyup', function () {
        ...
        const rows = document.querySelectorAll('#trashedLocationsTable tbody tr, .trashed-location-card');
